...
class feature_extraction(nn.Module):
    def __init__(self, concat_feature=False, concat_feature_channel=12, out_add=True, key_query_same=True, deformable_groups=2):
        ...
        self.imgGar = simple_attn(32, 2)
        self.down = nn.Conv2d(32, 32, 3, 2, 1, 1)
...
class GwcNet(nn.Module):
    def __init__(self, maxdisp, start_disp, end_disp, out_add, key_query_same, deformable_groups, output_representation, use_concat_volume=True):
    ...
    def query(self, points):
        # points: [bs, 3, 50000] labels: [bs, 1, 50000]
        ...
        dis = self.end_disp - self.start_disp
        ...
        if self.output_representation == "bimodal":
            ...
            # Rescale outputs
            self.preds = [self.disp * dis + self.start_disp,
                          self.mu0 * dis + self.start_disp,
                          self.mu1 * dis + self.start_disp,
    ...
    def forward(self, left, right, points, scale, o_shape = None):
        ...
        # [16, 40, 48, 64, 128]
        gwc_volume = build_gwc_piancha_dynamic_interp_volume_range(features_left["gwc_feature"], features_right["gwc_feature"], self.maxdisp // 4,
                                                                   self.num_groups, scale, self.start_disp, self.end_disp)
        if self.use_concat_volume:
            # concat_volume: [B, C, disparity, H, W]
            # [16, 24, 48, 64, 128]
            concat_volume = build_concat_dynamic_interp_volume_range(features_left["concat_feature"], features_right["concat_feature"],
                                                self.maxdisp // 4, scale, self.start_disp, self.end_disp)
            # [16, 64, 48, 64, 128]
        ...
        # torch.squeeze(cost3,1): [bs, 48, 64, 128]  features_left["concat_feature"]: [bs, 12, 64, 128]
...
def GwcNet_GC_DispNO_Range(d, start_disp, end_disp, out_add, key_query_same, deformable_groups, output_representation):
    return GwcNet(d, start_disp, end_disp, out_add, key_query_same, deformable_groups, output_representation, use_concat_volume=True)