...
def test():
    os.makedirs('./predictions_hasDisp_origin', exist_ok=True)
    for batch_idx, sample in enumerate(TestImgLoader):
        ...
        for disp_est, top_pad, right_pad, fn in zip(disp_est_np, top_pad_np, right_pad_np, left_filenames):
            ...
            fn = os.path.join("predictions_hasDisp_origin", fn.split('/')[-1])
...
def test_sample(sample):
    ...
    imgL, imgR, s, o_shape = sample['left'], sample['right'], sample['s'], sample['o_shape']
    imgL = imgL.cuda()
    imgR = imgR.cuda()
    s = s.cuda()
    o_shape = o_shape.cuda()

    disp_ests = model(imgL, imgR, s.item(), o_shape)
    return disp_ests[0]
