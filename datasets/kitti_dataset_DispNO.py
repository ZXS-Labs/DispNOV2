...
import numpy as np
import torch
from datasets.data_io import get_transform, read_all_lines, get_boundaries, scale_coords

# KITTI 1242 * 375
class KITTIDatasetDispNO(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            ...
            if w - crop_w < 0:
                ...
                s = w / self.lr_width
                crop_w = w
                crop_h = round(self.lr_height * s)
                random_y = h - crop_h
            else:
                random_x = w - crop_w
                random_y = h - crop_h
            x1 = random.randint(0, random_x)
            y1 = random.randint(0, random_y)
            ...
            samples = torch.from_numpy(samples) # torch.Size([3, 128 * 64])
            ...
            remain_w = w % int(4 * s)
            remain_h = h % int(4 * s)
            pad_w = w + int(4 * s) - remain_w
            pad_h = h + int(4 * s) - remain_h
