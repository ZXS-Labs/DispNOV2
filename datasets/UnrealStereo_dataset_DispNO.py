...
import numpy as np
from datasets.data_io import get_transform, read_all_lines, pfm_imread, get_boundaries, scale_coords, npy_imread
...
class UnrealStereoDatsetDispNO(Dataset):
    ...
    def sampling(self, disparity):
        ...
        if self.sampler == "random":
            random_points = self.__get_coords(gt) #（disparity != 0的点的个数, 3）
            idx = np.random.choice(random_points.shape[0], self.num_sample_inout) # (50000)
            points = random_points[idx, :] # (50000, 3) 分别为u v d
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            ...
            return {"left": left_img, # torch.Size([3, 256, 512])
                    "right": right_img,
            ...
            remain_w = crop_w % (4 * int(s))
            remain_h = crop_h % (4 * int(s))
            pad_w = crop_w + (4 * int(s)) - remain_w
            pad_h = crop_h + (4 * int(s)) - remain_h
            
            top_pad = pad_h - crop_h
            right_pad = pad_w - crop_w
            if top_pad > 0 and right_pad > 0:
                left_p = Image.new('RGB', (pad_w, pad_h), (0, 0, 0))
                right_p = Image.new('RGB', (pad_w, pad_h), (0, 0, 0))
                left_p.paste(left_img, (0, top_pad, crop_w, pad_h))
                right_p.paste(right_img, (0, top_pad, crop_w, pad_h))
            else:
                left_p = left_img
                right_p = right_img
            ...
            return {"left": left_img, # torch.Size([1, 3, 512, 960])
                    "right": right_img,
                    "disparity": disparity, # (512, 960)
                    "points": coords, # torch.Size([491520(512 * 960), 2])
                    "o_shape": o_shape, # torch.Size([2])  val: tensor([512, 960])
                    "scale": s,
                    "top_pad": top_pad,
                    "right_pad": right_pad,
