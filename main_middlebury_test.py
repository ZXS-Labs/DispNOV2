...
def test():
    os.makedirs('./pred/middlebury_hda_0_400_scale4', exist_ok=True)
    ...
    for batch_idx, sample in enumerate(TestImgLoader):
        ...
        for disp_est, top_pad, right_pad, fn in zip(disp_est_np, top_pad_np, right_pad_np, left_filenames):
            ...
            save_path = "pfm_output/trainingH/" + fn.split('/')[-2]
            if not os.path.exists(save_path):
                os.mkdir(save_path)
            fileName = "pfm_output/trainingH/" + fn.split('/')[-2] + "/disp0HDA.pfm"
            writePFM(fileName, disp_vis)
            disp_est = np.array(disp_est[top_pad:, :-right_pad], dtype=np.float32)
            fn = os.path.join("pred/middlebury_hda_0_400_scale4", fn.split('/')[-2] + '.png')
