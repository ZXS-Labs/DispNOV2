...
def test():
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        # test
        os.makedirs('./predictions_middlebury_DispNO_15_303_scale5', exist_ok=True)
        ...
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            for disp_est, top_pad, right_pad, fn in zip(disp_est_np, top_pad_np, right_pad_np, left_filenames):
                ...
                save_path = "output/trainingH/" + fn.split('/')[-2]
                if not os.path.exists(save_path):
                    os.makedirs(save_path, exist_ok=True)
                fileName = "output/trainingH/" + fn.split('/')[-2] + "/disp0DispNO.pfm"
                writePFM(fileName, disp_vis)
                disp_est = np.array(disp_est[top_pad:, :-right_pad], dtype=np.float32)
                fn = os.path.join("predictions_middlebury_DispNO_15_303_scale5", fn.split('/')[-2] + '.png')
